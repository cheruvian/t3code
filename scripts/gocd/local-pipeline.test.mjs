...
it("rejects a build missing only the Electron executable without publishing it", async () => {
  const sandbox = mkdtempSync(join(tmpdir(), "t3-gocd-missing-electron-build-"));
  const runtimeRoot = join(sandbox, "runtime");
  const artifactRoot = join(sandbox, "artifact");
  ...
  try {
    process.env.T3_PIPELINE_RUNTIME_ROOT = runtimeRoot;
    process.env.T3_PIPELINE_ARTIFACT_ROOT = artifactRoot;
    const { build } = await import("./local-pipeline.mjs?missing-electron-build-test");

    await expect(
      build("production", {
        assembleRelease: (pending) => {
          pendingRelease = pending;
          createCompleteRelease(pending, sha);
          rmSync(electronExecutablePath(pending));
        },
        resolveCommit: () => sha,
      }),
    ).rejects.toThrow("Build did not produce a complete release");
...
it("materializes Electron after deploying the desktop release", async () => {
  const pendingRelease = join("pending", "release");
  const { assembleRunPlan } = await import("./local-pipeline.mjs?assemble-run-plan-test");
  const commands = assembleRunPlan(pendingRelease);
  const desktopDeployIndex = commands.findIndex(
    ([command, args]) =>
      command === "pnpm" &&
      args.includes("@t3tools/desktop") &&
      args.at(-1) === join(pendingRelease, "apps", "desktop"),
  );
  const ensureIndex = commands.findIndex(
    ([command, args]) =>
      command === "node" &&
      args.length === 1 &&
      args[0] === join(pendingRelease, "apps", "desktop", "scripts", "ensure-electron-runtime.mjs"),
  );

  assert.notEqual(desktopDeployIndex, -1);
  assert.notEqual(ensureIndex, -1);
  assert.ok(ensureIndex > desktopDeployIndex);
});

it("clears a desktop subtree populated by the server deploy", async () => {
  const sandbox = mkdtempSync(join(tmpdir(), "t3-gocd-desktop-deploy-target-"));
  const pendingRelease = join(sandbox, "release");
  const desktopRelease = join(pendingRelease, "apps", "desktop");

  try {
    mkdirSync(desktopRelease, { recursive: true });
    writeFileSync(join(desktopRelease, "server-deploy-sentinel"), "stale");
    const { prepareDesktopDeployTarget } =
      await import("./local-pipeline.mjs?prepare-desktop-deploy-target-test");

    prepareDesktopDeployTarget(pendingRelease);

    assert.equal(existsSync(desktopRelease), false);
  } finally {
    rmSync(sandbox, { recursive: true, force: true });
  }
});
