...
Configure a YAML config repository pointing at this repository's `main` branch and matching `**/*.gocd.yaml`. This loads `t3code-candidate.gocd.yaml` and `t3code-main.gocd.yaml`. Change the Git material if this fork uses another remote. GoCD promotes committed material, not uncommitted worktree changes.
