...
describe("resolveInheritedProjectScripts", () => {
  const fileScript = (
    name: string,
    command: string,
  ...
  it("lets each higher layer override lower layers", () => {
    ...
    expect(resolveInheritedProjectScripts([project], [file], [], [])).toEqual([project]);
    expect(resolveInheritedProjectScripts([], [file], [global], [])).toEqual([
  ...
  it("excludes disabled inherited actions and restores them when ids are removed", () => {
    ...
    expect(
      resolveInheritedProjectScripts([], [file], [global], [normalizedFile.id, global.id]),
    ).toEqual([]);
    expect(resolveInheritedProjectScripts([], [file], [global], [])).toEqual([
      normalizedFile,
      global,
  ...
  it("never disables project actions and keeps them when inherited actions reuse identity", () => {
    ...
    expect(resolveInheritedProjectScripts([project], [], [global], [project.id])).toEqual([
      project,
    ]);
  });
