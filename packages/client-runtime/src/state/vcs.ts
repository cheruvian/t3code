...
export function createVcsEnvironmentAtoms<R, E>(
  ...
  const listRefsFamily = Atom.family((key: string) => {
    ...
    return runtime
      ...
      .pipe(Atom.setIdleTTL(0), Atom.withLabel(`environment-data:vcs:list-refs:${key}`));
  });
  const listRefs = (target: {
    readonly environmentId: EnvironmentId;
    readonly input: VcsListRefsInput;
  }) =>
    listRefsFamily(JSON.stringify([target.environmentId, normalizeVcsListRefsInput(target.input)]));
