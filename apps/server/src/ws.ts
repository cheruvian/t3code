        ...
        loadServerConfig.pipe(
          Effect.flatMap((config) =>
            options.usageLimitsCommand
              ? usageLimitSources.current.pipe(
                  Effect.map((sources) => ({
                    ...config,
                    providers: withUsageLimitsCommands(config.providers, sources),
                  })),
                )
          ...
          observeRpcEffect(
            ...
            Effect.gen(function* () {
              ...
              const shouldStopSessionAfterCommand = archiveCommand
                ...
                : false;
              const dispatch = dispatchNormalizedCommand(normalizedCommand).pipe(
                ...
                yield* Effect.forkScoped(
                  assistantPreviewBus.stream.pipe(
                    ...
                    Stream.runForEach((publication) =>
                      liveBuffer.offer({ kind: "preview", preview: publication.preview }),
                    ),
                    Effect.raceFirst(liveBuffer.failed),
