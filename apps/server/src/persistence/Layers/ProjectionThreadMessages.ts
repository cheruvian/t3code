...
import {
  ...
  ProjectionThreadMessageIdentity,
...
const makeProjectionThreadMessageRepository = Effect.gen(function* () {
  ...
  const getMessageIdentity = SqlSchema.findOneOption({
    Request: GetProjectionThreadMessageInput,
    Result: ProjectionThreadMessageIdentity,
    execute: ({ messageId }) =>
      sql`SELECT thread_id AS "threadId", role, created_at AS "createdAt" FROM projection_thread_messages WHERE message_id = ${messageId}`,
  });
  ...
  return {
    ...
    appendStreaming,
    getByMessageId,
    getIdentityByMessageId: (input) =>
      getMessageIdentity(input).pipe(
        Effect.mapError(
          toPersistenceSqlError("ProjectionThreadMessageRepository.getIdentityByMessageId:query"),
        ),
      ),
