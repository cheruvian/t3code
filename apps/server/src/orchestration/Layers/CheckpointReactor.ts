...
const make = Effect.gen(function* () {
  ...
  const resolveCheckpointCwd = Effect.fn("resolveCheckpointCwd")(function* (input: {
    ...
    const sessionCwd = Option.getOrUndefined(Option.map(fromSession, (runtime) => runtime.cwd));
    const candidates = input.preferSessionRuntime
      ? [sessionCwd, fromThread]
      : [fromThread, sessionCwd];
    for (const cwd of candidates) {
      if (cwd && (yield* checkpointStore.isGitRepository(cwd))) return cwd;
    }
    return undefined;
  });
  ...
  const processRuntimeEvent = Effect.fn("processRuntimeEvent")(function* (
    ...
    if (event.type === "turn.started") {
      const turnId = toTurnId(event.turnId);
      const activeTurnId = Option.getOrUndefined(
        yield* providerService
          .getSession(event.threadId)
