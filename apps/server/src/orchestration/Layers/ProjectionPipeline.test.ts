...
it.layer(Layer.fresh(makeProjectionPipelinePrefixedTestLayer("t3-projection-cursor-batch-")))(
  ...
  (it) => {
    it.effect("validates cursors then writes a project and all cursors in three statements", () =>
      Effect.gen(function* () {
    ...
    Effect.gen(function* () {
      ...
      const stateRows = yield* sql<{
        ...
        SELECT
          projector,
          last_applied_sequence AS "lastAppliedSequence"
        FROM projection_state WHERE projector != 'projection.attachment-cleanup'
      `;
