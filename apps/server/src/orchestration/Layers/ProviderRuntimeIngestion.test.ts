...
function providerServiceFromCodexAdapter(
  ...
  return {
    ...
    compactThread: () => Effect.die("unsupported compaction"),
    ...
    getSession: (threadId) =>
      adapter.getSession
        ? adapter.getSession(threadId)
        : adapter
            .listSessions()
            .pipe(
              Effect.map((sessions) =>
                Option.fromNullishOr(sessions.find((session) => session.threadId === threadId)),
              ),
            ),
...
describe("ProviderRuntimeIngestion", () => {
  ...
  async function createHarness(options?: {
    ...
    beforeRuntimeContextRead?: (threadId: ThreadId) => Effect.Effect<void>;
    ...
    const projectionSnapshotLayer = options?.beforeRuntimeContextRead
      ? Layer.effect(
          ProjectionSnapshotQuery,
          Effect.gen(function* () {
            ...
            return ProjectionSnapshotQuery.of({
              ...
              getThreadRuntimeContext: (threadId) =>
                options.beforeRuntimeContextRead!(threadId).pipe(
    ...
    const receiptBus = await testRuntime.runPromise(Effect.service(RuntimeReceiptBus));
    ...
    return {
      ...
      readEvents: () =>
        testRuntime.runPromise(
          Stream.runCollect(engine.readEvents(0)).pipe(Effect.map((events) => [...events])),
        ),
      readShellModel: () => testRuntime.runPromise(snapshotQuery.getShellSnapshot()),
  ...
  effectIt.effect("ingests activity bursts without lifecycle context reads", () =>
    Effect.gen(function* () {
      ...
      expect(pendingTurnStartReadCount).toBe(1);
    }),
  );
  ...
  effectIt.effect("uses ingestion capacity across independent threads", () =>
    Effect.gen(function* () {
      ...
      const harness = yield* Effect.promise(() =>
        createHarness({
          beforeRuntimeContextRead: (threadId) =>
            Effect.gen(function* () {
  ...
  effectIt.effect("serializes same-thread lifecycle reads", () =>
    Effect.gen(function* () {
      ...
      const harness = yield* Effect.promise(() =>
        createHarness({
          beforeRuntimeContextRead: (readThreadId) =>
            Effect.gen(function* () {
  ...
  it("honors project streaming overrides without rereading lifecycle context for each delta", async () => {
    let reads = 0;
    const harness = await createHarness({
      serverSettings: {
        responseStreamingMode: "turn",
        projectSettingsOverrides: {
          [asProjectId("project-1")]: { responseStreamingMode: "token" },
        },
      },
      beforeRuntimeContextRead: () =>
        Effect.sync(() => {
    ...
    const base = {
      provider: ProviderDriverKind.make("codex"),
      threadId: asThreadId("thread-1"),
      turnId: asTurnId("project-stream-turn"),
      createdAt: "2026-01-01T00:00:01.000Z",
    };
    harness.emit({ ...base, type: "turn.started", eventId: asEventId("project-stream-start") });
    for (const [index, delta] of ["Project ", "tokens"].entries()) {
      harness.emit({
        ...base,
        type: "content.delta",
        eventId: asEventId(`project-stream-${index}`),
        itemId: asItemId("project-stream-item"),
  ...
  it("buffers assistant deltas without querying persistence until completion", async () => {
    const harness = await createHarness();
    const now = "2026-01-01T00:00:00.000Z";
    ...
    expect(harness.sqlCount() - before).toBe(0);
