...
const PROVIDER_RUNTIME_INGESTION_CONCURRENCY = 4;
...
function sourceProposedPlanReference(pendingTurnStart: Option.Option<ProjectionPendingTurnStart>): {
  ...
  return sourceThreadId === null || sourcePlanId === null ? null : { sourceThreadId, sourcePlanId };
}
...
const make = Effect.gen(function* () {
  ...
  const threadProjectIds = yield* Cache.make({
    capacity: 10_000,
    ...
    lookup: (threadId: ThreadId) =>
      projectionSnapshotQuery
        ...
        .pipe(
          Effect.map(Option.match({ onNone: () => null, onSome: (thread) => thread.projectId })),
        ),
  });
  ...
  const resolveThreadRuntimeContext = Effect.fn("resolveThreadRuntimeContext")(function* (
    ...
    if (thread) yield* Cache.set(threadProjectIds, threadId, thread.projectId);
    return thread;
  });
  ...
  const resolveResponseStreamingMode = Effect.fn("resolveResponseStreamingMode")(function* (
    threadId: ThreadId,
  ) {
    const projectId = yield* Cache.get(threadProjectIds, threadId);
    const settings = yield* serverSettingsService.getSettings;
    return resolveProjectSettings(settings, projectId).settings.responseStreamingMode;
  });
  ...
  const processRuntimeEvent = (event: ProviderRuntimeEvent) =>
    Effect.gen(function* () {
      ...
      const thread = needsRuntimeContext ? yield* resolveThreadRuntimeContext(threadId) : undefined;
      if (needsRuntimeContext && !thread) return;
      ...
      const projectedEventTurn =
        eventTurnId === undefined || !isTerminalTurn
          ? Option.none()
          : yield* projectionTurnRepository.getByTurnId({ threadId, turnId: eventTurnId });
      ...
      const expectedProviderTurnId =
        ...
        (conflictsWithActiveTurn || pendingSourceProposedPlan !== null)
      ...
      const conflictingTurnStartIsPendingTurnStart =
        event.type === "turn.started" && conflictsWithActiveTurn
          ? sameId(expectedProviderTurnId, eventTurnId) && Option.isSome(pendingTurnStart)
          : false;
      ...
      if (reasoningDelta && reasoningDelta.delta.length > 0 && reasoningTurnId) {
        ...
        yield* rememberAssistantMessageId(threadId, turnId, reasoningMessageId);
        ...
        const streamingMode = yield* resolveResponseStreamingMode(threadId);
      ...
      if (assistantDelta && assistantDelta.length > 0) {
        ...
        if (turnId) {
          yield* rememberAssistantMessageId(threadId, turnId, assistantMessageId);
        }
      ...
      if (event.type === "item.completed" && event.payload.itemType === "reasoning") {
        ...
        if (turnId) {
          ...
          const existingReasoningMessage = Option.isSome(activeReasoningMessageId)
            ? yield* getThreadMessageById(threadId, activeReasoningMessageId.value)
            : undefined;
          const fallbackText =
          ...
          if (Option.isNone(activeReasoningMessageId)) {
            ...
            const turnAlreadyStreamedReasoning = Option.isSome(
              yield* getAssistantSegmentStateForTurn(threadId, turnId, "reasoning"),
            );
            ...
            if (fallbackText !== undefined && !turnAlreadyStreamedReasoning) {
              ...
              const existingSnapshot = yield* getThreadMessageById(threadId, snapshotMessageId);
      ...
      if (assistantCompletion) {
        ...
        const [existingAssistantMessage, hasAssistantMessagesForTurn] = yield* Effect.all([
          getThreadMessageById(threadId, assistantMessageId),
          turnId === undefined
        ...
        if (!shouldSkipRedundantCompletion) {
          ...
          if (turnId) {
            yield* forgetAssistantMessageId(threadId, turnId, assistantMessageId);
          }
        }
      ...
      if (isTerminalTurn) {
        ...
        if (turnId) {
          ...
          yield* Effect.forEach(
            assistantMessageIds,
            (assistantMessageId) =>
              getThreadMessageById(threadId, assistantMessageId).pipe(
          ...
          yield* clearAssistantSegmentStateForTurn(threadId, turnId);
          yield* clearAssistantSegmentStateForTurn(threadId, turnId, "reasoning");

          yield* finalizeBufferedProposedPlan({
            event,
            threadId: threadId,
            planId: proposedPlanIdForTurn(threadId, turnId),
            turnId,
            updatedAt: now,
          });
        }
      ...
      if (event.type === "session.exited") {
        yield* clearTurnStateForSession(threadId);
      }
      ...
      if (event.type === "task.started" || event.type === "task.progress") {
        ...
        if (description) {
          yield* rememberTaskDescription(threadId, event.payload.taskId, description);
        }
      }
      ...
      if (event.type === "session.exited") {
        threadPlanProgress.clearThreadPlanProgress(threadId);
      } else if (!conflictsWithActiveTurn) {
        if (event.type === "turn.plan.updated") {
          threadPlanProgress.recordPlanProgress(threadId, event.payload.plan);
      ...
      switch (event.type) {
        ...
        case "task.completed": {
          const payload = event.payload as {
            ...
            status?: string;
            agentId?: string;
            itemLifecycle?: "started" | "completed";
          };
          threadBackgroundLiveness.recordTaskLiveness({
            ...
            itemId: event.itemId,
            itemLifecycle: payload.itemLifecycle,
            kind:
      ...
      if (event.type === "task.completed") {
        taskTitle = yield* lookupTaskDescription(threadId, event.payload.taskId);
        if (!taskTitle) {
  ...
  const worker = yield* makeKeyedDrainableWorker({
    concurrency: PROVIDER_RUNTIME_INGESTION_CONCURRENCY,
    key: (input: RuntimeIngestionInput) =>
      input.source === "domain" ? input.event.payload.threadId : input.event.threadId,
    process: (input) => processInput(input).pipe(logIngestionFailure(input.source, input.event)),
  });
...
export const ProviderRuntimeIngestionLayer = Layer.effect(ProviderRuntimeIngestionService, make);
export const ProviderRuntimeIngestionLive = ProviderRuntimeIngestionLayer.pipe(
