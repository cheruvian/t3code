...
import { type TurnIngestionSettledReceipt } from "../Services/RuntimeReceiptBus.ts";
import {
  ProviderService,
...
describe("CheckpointReactor", () => {
  ...
  async function createHarness(options?: {
    ...
    const receipts = await Effect.runPromise(
      Effect.gen(function* () {
        ...
        yield* Stream.runForEach(receiptBus.streamEventsForTest, (receipt) =>
          receipt.type === "turn.ingestion-settled" ? Effect.void : Queue.offer(receipts, receipt),
    ...
    return {
      ...
      emitSettledTurn: (event: LegacyProviderRuntimeEvent) =>
        Effect.gen(function* () {
          provider.emit(event);
          if ((event.type === "turn.completed" || event.type === "turn.aborted") && event.turnId) {
            yield* receiptBus.publish({
              ...
              outcome: event.type === "turn.aborted" ? "interrupted" : "completed",
    ...
    (terminalEventType) =>
      Effect.gen(function* () {
        ...
        expect(thread?.checkpoints[0]?.status).toBe(
          terminalEventType === "turn.aborted" ? "missing" : "ready",
        );
