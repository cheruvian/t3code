...
export default function ChatView(props: ChatViewProps) {
  ...
  const activeProjectFileScripts = useT3ProjectFileScripts(
    environmentId,
    activeProject?.workspaceRoot ?? null,
  );
  const effectiveProjectScripts = useMemo(
    () =>
      resolveInheritedProjectScripts(
        activeProjectScripts,
        activeProjectFileScripts,
        settings.globalScripts,
        activeProject?.disabledInheritedScriptIds ?? [],
      ),
    [
      activeProjectScripts,
      activeProjectFileScripts,
  ...
  const configuredPreviewUrls = useMemo(
    () => getConfiguredPreviewUrls(effectiveProjectScripts),
    [effectiveProjectScripts],
  );
  ...
  useEffect(() => {
    const handler = (event: globalThis.KeyboardEvent) => {
      ...
      const script = effectiveProjectScripts.find((entry) => entry.id === scriptId);
      if (!script) return;
      event.preventDefault();
      event.stopPropagation();
  ...
  const rightPanelContent = activeThreadRef ? (
    renderedRightPanelSurface?.kind === "preview" ? (
      ...
      />
    ) : mountDiffPanel && renderedRightPanelSurface?.kind === "diff" ? (
      <Suspense fallback={null}>
        <DiffWorkerPoolProvider active>
          <DiffPanel
      ...
      >
        ...
        >
          ...
          <ChatHeader
            ...
            {...(activeProject
              ? { activeProjectScriptIds: new Set(activeProjectScripts.map((script) => script.id)) }
              : {})}
            preferredScriptId={
        ...
        <div className="flex min-h-0 min-w-0 flex-1">
          ...
          >
            ...
            <div className="relative flex min-h-0 flex-1 flex-col">
              ...
              <DiffWorkerPoolProvider
                active={shouldMountDiffWorkerPool({
                  turnDiffSummaryCount: activeThread.checkpoints.length,
                  rightPanelSurfaceKinds: [],
                })}
              >
                <MessagesTimeline
