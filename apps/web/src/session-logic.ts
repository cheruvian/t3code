...
export function deriveTimelineEntriesWithState(
  ...
  if (canAppend) {
    ...
    const suffix = sources.every(isTimelineSourceOrdered)
      ? mergeOrderedTimelineSources(sources)
      : sources.flat().toSorted(compareTimelineEntriesByCreatedAt);
    return {
      messages,
      proposedPlans,
      workEntries,
  ...
  return {
    ...
    proposedPlans,
    workEntries,
    entries: [messageRows, proposedPlanRows, workRows].every(isTimelineSourceOrdered)
      ? mergeOrderedTimelineSources([messageRows, proposedPlanRows, workRows])
...
export function deriveTimelineEntries(
  ...
  return mergeTurnPlanEntries(
    deriveTimelineEntriesWithState(messages, proposedPlans, workEntries).entries,
    turnPlans,
  );
}

export function mergeTurnPlanEntries(
  entries: TimelineEntry[],
  turnPlans: ReadonlyArray<TurnPlanEntry>,
): TimelineEntry[] {
  if (turnPlans.length === 0) return entries;
  const planEntries = turnPlans.map((turnPlan): TimelineEntry => ({
    id: turnPlan.id,
    kind: "turn-plan",
  ...
  if (
    planEntries.some(
      (entry, index) =>
        index > 0 && compareTimelineEntriesByCreatedAt(planEntries[index - 1]!, entry) > 0,
    )
  )
    planEntries.sort(compareTimelineEntriesByCreatedAt);
  return mergeTimelineEntrySuffix(entries, planEntries);
}
